...
<script>
    ...
    const mentorDocs = {
        art: "<h3>Artifact Mismatch</h3><p>Girdiğiniz kanıtlar sistem loglarıyla eşleşmiyor. <strong>Logs</strong> sekmesine gidip doğru değeri bulmalısınız.</p>",
        auth: "<h3>Unauthorized Action</h3><p>Ransomware gibi kritik vakaları L1 analistler kapatamaz. Mutlaka <strong>Escalate</strong> etmelisiniz.</p>",
        logic: "<h3>Logic Conflict</h3><p>Saldırı olduğu kesinleşen bir vakaya 'False Positive' diyemezsiniz.</p>"
    };
    ...
    function openCase(k) {
        ...
        if(c.status === 'open') {
            ...
            b.innerHTML = `<div class="card"><h4>Archive Report</h4><p>Verdict: ${c.verdict}</p><hr style="border:0; border-top:1px solid #30363d; margin:15px 0;"><p>${c.report}</p></div>`;
