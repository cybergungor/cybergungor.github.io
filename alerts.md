...
<style>
    /* MONITOR FRAME & WORKSTATION DESIGN */
    main { background: #010409; padding-bottom: 50px !important; }
    ...
    .monitor-frame {
        ...
        box-shadow: 0 50px 100px rgba(0,0,0,0.8), inset 0 0 20px rgba(0,0,0,1);
        position: relative;
    }
    ...
    /* SOC UI STYLES */
    .soc-wrapper { display: flex; flex-direction: column; height: 750px; color: #e6edf3; font-family: 'Inter', sans-serif; position: relative; }
    
    #toast-container { position: absolute; top: 20px; left: 50%; transform: translateX(-50%); z-index: 11000; display: flex; flex-direction: column; align-items: center; gap: 8px; width: 80%; }
    #toast { background: #161b22; border: 1px solid #58a6ff; color: #fff; padding: 12px 25px; border-radius: 8px; font-family: 'JetBrains Mono'; font-size: 0.8rem; box-shadow: 0 10px 30px rgba(0,0,0,0.5); display: none; text-align: center; }
    ...
    .modal-box { background: #0d1117; border: 1px solid #30363d; width: 90%; border-radius: 12px; overflow: hidden; }
    ...
    .incident-table th { background: #161b22; text-align: left; padding: 15px; font-size: 0.7rem; color: #8b949e; text-transform: uppercase; position: sticky; top: 0; }
    ...
    .side-panel { position: absolute; top: 0; right: -100%; width: 100%; height: 100%; background: #0d1117; transition: 0.4s cubic-bezier(0.05, 0.7, 0.1, 1); z-index: 9000; display: flex; flex-direction: column; }
    ...
    /* LABS INTRO DESIGN */
    .labs-intro { padding: 30px; background: linear-gradient(145deg, #0d1117 0%, #161b22 100%); border: 1px solid #30363d; border-radius: 12px; margin-bottom: 40px; max-width: 1200px; margin-left: auto; margin-right: auto; }
</style>

<div class="labs-intro">
    <div style="display: flex; align-items: center; gap: 15px; margin-bottom: 15px;">
        <div style="width: 12px; height: 12px; background: #238636; border-radius: 50%; box-shadow: 0 0 10px #238636;"></div>
        <h2 style="margin: 0; color: #58a6ff; font-family: 'JetBrains Mono'; font-size: 1.4rem;">LAB-01: SentinelGuard SOC Simulator</h2>
    </div>
    <p style="color: #8b949e; line-height: 1.6; font-size: 0.95rem; margin-bottom: 20px;">
        SentinelGuard, modern bir Güvenlik Operasyonları Merkezi (SOC) iş akışını test etmek için tasarlanmış <strong>etkileşimli bir olay müdahale simülatörüdür.</strong> Analiz yeteneklerinizi gerçek dünya senaryoları üzerinde deneyimleyin.
    </p>
    <div style="display: grid; grid-template-columns: repeat(auto-fit, minmax(250px, 1fr)); gap: 15px;">
        <div style="background: rgba(88, 166, 255, 0.05); border: 1px solid rgba(88, 166, 255, 0.2); padding: 12px; border-radius: 8px;">
            <h4 style="margin: 0 0 5px 0; color: #58a6ff; font-size: 0.8rem;">🔍 LOG INVESTIGATION</h4>
            <p style="margin: 0; font-size: 0.75rem; color: #8b949e;">SIEM yeteneklerini kullanarak 500+ log arasından delilleri ayıklayın.</p>
        </div>
        <div style="background: rgba(210, 153, 34, 0.05); border: 1px solid rgba(210, 153, 34, 0.2); padding: 12px; border-radius: 8px;">
            <h4 style="margin: 0 0 5px 0; color: #d29922; font-size: 0.8rem;">🛡️ INCIDENT LIFECYCLE</h4>
            <p style="margin: 0; font-size: 0.75rem; color: #8b949e;">Triage, Analysis ve Resolution aşamalarını profesyonelce yönetin.</p>
        </div>
    </div>
</div>
        ...
        <div class="screen-container">
            ...
            <div class="soc-wrapper">
                ...
                <div id="mentorModal" class="modal-overlay">
                    <div class="modal-box">
                        <div class="modal-header" style="background:#161b22; padding:20px; border-bottom:1px solid #30363d; color:#f85149; text-align:center; font-weight:bold;">ANALYST MENTOR // SYSTEM REASONING</div>
                        <div class="modal-body" id="mentorBody" style="padding:25px; color:#8b949e; line-height:1.6;"></div>
                        <div style="padding:15px; text-align:right; border-top:1px solid #30363d;"><button class="btn-filter" style="background:#58a6ff; color:#000" onclick="closeModal('mentorModal')">I UNDERSTAND</button></div>
                    </div>
                </div>
                ...
                <div style="flex:1; overflow-y:auto; background:#0d1117;">
                ...
                <div id="sidePanel" class="side-panel">
                    <div style="background:#161b22; padding:20px; border-bottom:1px solid #30363d; display:flex; justify-content:space-between; align-items:center;">
                        <span id="pID" style="color:#58a6ff; font-weight:bold; font-family:'JetBrains Mono';">#SOC-0000</span>
                        <span onclick="closePanel()" style="cursor:pointer; font-size:24px; color:#8b949e;">×</span>
                    </div>
...
<script>
    ...
    function checkArtifacts(k) {
        ...
        cases[k].q.forEach((q,i) => {
            ...
            if(input.value.trim().toLowerCase() !== q.a.toLowerCase()) { valid = false; input.style.borderColor = "#f85149"; } 
            else { input.style.borderColor = "#238636"; }
        });
        return valid;
    }
    ...
    function doEsc(k) {
        if(!checkArtifacts(k)) { showToast("ESC FAILED: Invalid artifacts provided.", "art"); return; }
        const v = document.getElementById('vSel').value;
    ...
    function switchTab(t) { currentTab = t; document.getElementById('tab-open-btn').classList.toggle('active', t === 'open'); document.getElementById('tab-closed-btn').classList.toggle('active', t === 'closed'); renderTable(); }
