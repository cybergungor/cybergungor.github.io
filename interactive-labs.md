...
<style>
    ...
    /* 4. WINDOWS (TIKLANMA SORUNU İÇİN POINTER-EVENTS: AUTO) */
    .window {
    ...
    /* 5. LAB SELECTION CARDS */
    .lab-card { background:#161b22; border:1px solid #30363d; padding:20px; border-radius:10px; cursor:pointer; transition: 0.3s; pointer-events: auto; }
    .lab-card:hover { border-color: #58a6ff; background: rgba(88, 166, 255, 0.05); }

    /* 6. LAB CONTENT & TERMINAL */
    ...
    .edr-terminal { background: #000; color: #3fb950; padding: 10px; font-family: 'Courier New', monospace; height: 115px; overflow-y: auto; border: 1px solid #30363d; margin-top: 10px; border-radius: 4px; font-size: 11px; }

    /* 7. UI ELEMENTS */
    .btn-action { background: #238636; border: none; color: white; padding: 12px 20px; border-radius: 6px; cursor: pointer; font-weight: bold; width: 100%; pointer-events: auto; }
    .btn-escalate { background: #d29922; margin-top: 10px; }
...
<div id="desktop-environment">
    <div id="system-modal" class="modal-overlay hidden" style="position: absolute; top: 0; left: 0; width: 100%; height: 100%; background: rgba(0,0,0,0.9); z-index: 2000; display: flex; justify-content: center; align-items: center; pointer-events: auto;">
        <div style="background: #161b22; border: 1px solid #58a6ff; padding: 30px; border-radius: 12px; text-align: center; max-width: 500px;">
    ...
    <div id="auth-window" class="window">
        <div class="window-header"><span>Authentication</span><span onclick="closeWindow('auth-window')" style="cursor:pointer;">×</span></div>
    ...
    <div id="selection-window" class="window hidden">
        <div class="window-header"><span>Mission Control</span><span onclick="closeWindow('selection-window')" style="cursor:pointer;">×</span></div>
        <div id="selection-content" style="padding: 25px; overflow-y: auto;">
            <h4 style="color: #58a6ff; border-bottom: 1px solid #30363d; padding-bottom: 10px;">5 AVAILABLE MISSIONS</h4>
        ...
        <div id="briefing-content" class="hidden" style="padding: 25px; overflow-y: auto;">
            ...
            <div style="display: flex; gap: 15px;">
                ...
                <button onclick="startInvestigation()" class="btn-action">ENTER VAULT</button>
            </div>
        </div>
    </div>

    <div id="lab-window" class="window hidden">
        <div class="window-header"><span id="lab-title-text">Investigation</span><span onclick="closeWindow('lab-window')" style="cursor:pointer;">×</span></div>
    ...
    <div id="playbook-window" class="window hidden">
        <div style="padding: 30px; overflow-y: auto;">
            <h2 style="color: #3fb950; margin-top: 0;">DECISION CENTER</h2>
...
<script>
    // NOISE GENERATOR
    function getNoise(count) {
        let n = []; for(let i=0; i<count; i++) n.push(`[${10}:${Math.floor(Math.random()*59)}:12] INFO Sys: source=10.0.0.${i} user=analyst status=Success`);
        return n;
    }
    ...
    const labs = {
        brute: {
            ...
            logs: () => {
                let l = getNoise(100);
                l.push(`[14:40:05] WARN Auth: EventID=4625 source=141.10.22.5 user=svc_web status=Failure`);
                l.push(`[15:42:01] CRIT Auth: EventID=4624 source=141.10.22.5 user=svc_web status=Success`);
                return l.sort();
            },
        ...
        traversal: {
            ...
            questions: [{q: "Attacker IP?", a: "192.168.55.12"}, {q: "File?", a: "/etc/passwd"}],
            logs: () => {
                let l = getNoise(80);
        ...
        log4j: {
            name: "ADV-04: Log4Shell Injection",
        ...
        exfil: {
            ...
            questions: [{q: "Source?", a: "DESKTOP-AF2"}, {q: "Domain?", a: "dns-tunnel.com"}],
            logs: () => {
                let l = getNoise(100);
                l.push(`[11:05:00] ALERT IDS: DNS Tunneling detected for dns-tunnel.com from DESKTOP-AF2`);
                return l.sort();
            },
    ...
    function renderSelection() {
        ...
        container.innerHTML = Object.keys(labs).map(key => `<div class="lab-card" onclick="showBriefing('${key}')">${labs[key].name}</div>`).join('');
    }

    function showBriefing(key) { currentKey = key; document.getElementById('selection-content').classList.add('hidden'); document.getElementById('briefing-content').classList.remove('hidden'); document.getElementById('brief-details').innerHTML = "Mission Briefing for " + labs[key].name; }
    ...
    function startInvestigation() {
        ...
        document.getElementById('q-area').innerHTML = labs[currentKey].questions.map((q, i) => `<div style="margin-bottom:10px;"><label style="font-size:10px; color:#8b949e;">${q.q}</label><input type="text" id="ans-${i}" class="input-field"></div>`).join('');
    }

    function displayLogs(data) { document.getElementById('log-screen').innerHTML = data.map(l => `<div style="border-bottom:1px solid #161b22; padding:2px 0;">${l}</div>`).join(''); }
    function filterLogs() { const term = document.getElementById('log-search-input').value.toLowerCase(); const filtered = activeLogs.filter(l => l.toLowerCase().includes(term)); displayLogs(filtered); }

    function checkAnswers() {
        const lab = labs[currentKey]; let correct = 0;
        lab.questions.forEach((q, i) => { const el = document.getElementById(`ans-${i}`); if(el.value.trim().toLowerCase() === q.a.toLowerCase()) { correct++; el.classList.add('correct-ans'); } else { el.classList.add('wrong-ans'); } });
        if(correct === lab.questions.length) { openModal("VERIFIED", "Evidence confirmed. Choose decision."); setTimeout(() => { closeWindow('lab-window'); openWindow('playbook-window'); renderPlaybook(); }, 1500); }
    }

    function renderPlaybook() {
        document.getElementById('playbook-ui').innerHTML = `
            <select id="case-decision" class="input-field" onchange="toggleActions()">
                <option value="tp">True Positive (Escalate)</option>
                <option value="fp">False Positive (Close Case)</option>
            </select>
            <div id="tp-area" style="margin-top:20px;">
                <div class="edr-terminal"><div id="edr-out">[READY] Recommended: ${labs[currentKey].remedy}</div><input type="text" id="edr-in" onkeydown="handleEDR(event)" style="background:none; border:none; color:#3fb950; outline:none; width:100%;"></div>
    ...
    function toggleActions() { const v = document.getElementById('case-decision').value; document.getElementById('tp-area').className = v==='tp'?'':'hidden'; document.getElementById('fp-area').className = v==='fp'?'':'hidden'; }
    ...
    function finalize(t) { if(!document.getElementById('analyst-note').value) { alert("Note required."); return; } openModal("DONE", t==='tp'?"Escalated.":"Closed."); setTimeout(()=>location.reload(), 3000); }
